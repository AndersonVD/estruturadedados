            ...
            for (int i = 0; i < numAeronavesAterrissagem; i++) {
                if (filaAterrissagemPista1.size() <= filaAterrissagemPista2.size()) {
                    if (filaAterrissagemPista1.size() < MAX_PRATELEIRAS) {
                        ...
                        System.out.println("Avião " + aeronave.getId() + " entrou na fila de aterrissagem na pista 1 com " + aeronave.getCombustivel() + " unidades de combustível.");
                    }
                } else {
                    if (filaAterrissagemPista2.size() < MAX_PRATELEIRAS) {
                        ...
                        System.out.println("Avião " + aeronave.getId() + " entrou na fila de aterrissagem na pista 2 com " + aeronave.getCombustivel() + " unidades de combustível.");
                    }
                }
            }

// Chegam novas aeronaves para decolagem
int numAeronavesDecolagem = gerador.nextInt(3); // Gera número aleatório de 0 a 2
for (int i = 0; i < numAeronavesDecolagem; i++) {
if (totalAv < CAPACIDADE_MAXIMA_FILA * (NUMERO_FILAS_ATERRISSAGEM + NUMERO_FILAS_DECOLAGEM)) {
Aeronave aeronave = new Aeronave(totalAv + 1, true, gerador.nextInt(MAX_RESERVA_COMBUSTIVEL) + 1);
int filaMenor = 0;
...
if (aeronave.getReservaCombustivel() == 0 || pista.podePousar(aeronave.getReservaCombustivel())) {
...
if (tempo - aeronave.getTempoChegada() >= tempoEsperaDecolagem[aeronave.getId() % 2]) {
...
System.out.println("Aeronave " + aeronave.getId() + " pousando de reserva de combustível na pista 1.");
pista1Ocupada = true;
...
if (aeronave.getCombustivel() <= 0) {
System.out.println("Aeronave " + aeronave.getId() + " pousando de reserva de combustível na pista 2.");
pista2Ocupada = true;
numAterrissagensReserva++;
} else {
...
System.out.println("Aeronave " + pista1Aeronave.getId() + " pousou na pista 1.");
pista1Ocupada = false;
pista1Inicio = -1;
pista1Aeronave = null;
}
...
System.out.println("Aeronave " + pista2Aeronave.getId() + " pousou na pista 2.");
pista2Ocupada = false;
pista2Inicio = -1;
pista2Aeronave = null;
}
...
System.out.println("Aeronave " + pista1Aeronave.getId() + " decolou da pista 1.");
pista1Ocupada = false;
pista1Inicio = -1;
pista1Aeronave = null;
}
...
System.out.println("Aeronave " + pista2Aeronave.getId() + " decolou da pista 2.");
pista2Ocupada = false;
pista2Inicio = -1;
pista2Aeronave = null;
}
...
System.out.println("Fila de espera para aterrissagem na pista 1: " + filaAterrissagem1
